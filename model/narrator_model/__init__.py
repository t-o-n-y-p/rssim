...
class NarratorModel(MapBaseModel, ABC):
    ...
    def on_update_time(self, dt):
        ...
        if self.game_time >= self.playback_start_time + 1 and self.is_playing_announcement:
            self.view.on_announcement_play(self.narrator_queue[0][ANNOUNCEMENT_TEXT])
            self.narrator_queue.pop(0)

    @final
    def on_dt_multiplier_update(self, dt_multiplier):
        for announcement in [a for a in self.narrator_queue
                             if a[ANNOUNCEMENT_TYPE] in get_announcement_types_diff(self.dt_multiplier, dt_multiplier)]:
    ...
    def on_announcement_add(self, announcement_time, announcement_type, announcement_text):
        self.narrator_queue.append(
            (
                announcement_time, announcement_type in get_announcement_types_enabled(self.dt_multiplier),
                announcement_type, announcement_text
            )
        )
