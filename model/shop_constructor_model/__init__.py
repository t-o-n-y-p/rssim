...
from database import USER_DB_CURSOR, CONFIG_DB_CURSOR, LOCKED, UNDER_CONSTRUCTION, CONSTRUCTION_TIME, \
    UNLOCK_CONDITION_FROM_LEVEL, UNLOCK_CONDITION_FROM_PREVIOUS_STAGE, UNLOCK_AVAILABLE, STORAGE_CAPACITY, \
    SECONDS_IN_ONE_HOUR, HOURLY_PROFIT, EXP_BONUS, FALSE, TRUE, LEVEL_REQUIRED, MAX_CONSTRUCTION_TIME, PRICE
from model import MapBaseModel, SHOP_STORAGE_ALMOST_FULL_THRESHOLD


class ShopConstructorModel(MapBaseModel, ABC):
    def __init__(self, controller, view, map_id, shop_id):
        super().__init__(
            controller, view, map_id, logger=getLogger(f'root.app.game.map.{map_id}.shop.{shop_id}.constructor.model')
        )
        ...
        USER_DB_CURSOR.execute(
            ...
            FROM shop_stages WHERE map_id = ? AND shop_id = ?''', (self.map_id, self.shop_id)
        )
    ...
    def on_save_state(self):
        USER_DB_CURSOR.execute(
            '''UPDATE shops SET current_stage = ?, shop_storage_money = ?, internal_shop_time = ? 
            WHERE map_id = ? AND shop_id = ?''',
            (self.current_stage, self.shop_storage_money, self.internal_shop_time, self.map_id, self.shop_id)
        )
        for stage_number in self.shop_stages_state_matrix:
            USER_DB_CURSOR.execute(
                '''UPDATE shop_stages SET locked = ?, under_construction = ?, construction_time = ?, 
                unlock_condition_from_level = ?, unlock_condition_from_previous_stage = ?, unlock_available = ?
                ...
                (
                    ...
                    self.shop_stages_state_matrix[stage_number][UNLOCK_CONDITION_FROM_PREVIOUS_STAGE],
    ...
    def on_update_time(self, dt):
        if self.current_stage > 0 \
            ...
            if self.internal_shop_time // SECONDS_IN_ONE_HOUR \
                ...
                self.shop_storage_money += min(
                    self.shop_stages_state_matrix[self.current_stage][HOURLY_PROFIT],
                    self.shop_stages_state_matrix[self.current_stage][STORAGE_CAPACITY] - self.shop_storage_money
                )
