...
class RSSim:
    ...
    def check_for_updates(self):
        self.logger.info('START RSSIM.CHECK_FOR_UPDATES')
        ...
        self.logger.debug(f'user DB version: {user_db_version}')
        self.logger.debug(f'current game version: {current_version}')
        if self.user_db_cursor.fetchone() < current_version:
            self.logger.debug('upgrading database...')
            for patch in range(2, current_version[2] + 1):
                self.logger.debug(f'start 0.9.{patch} migration')
                with open(f'db/patch/09{patch}.sql', 'r') as migration:
                    for line in migration.readlines():
                        self.user_db_cursor.execute(line)
                        self.logger.debug(f'executed request: {line}')

                self.user_db_connection.commit()
                self.logger.debug(f'0.9.{patch} migration complete')
        else:
            self.logger.debug('user DB version is up to date')

        self.logger.info('END RSSIM.CHECK_FOR_UPDATES')
