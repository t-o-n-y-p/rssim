...
void main()
{
    ...
    vec2 top_left_cell = vec2(int(80.0 / 1280.0 * float(screen_resolution[0])), screen_resolution[1] - 111 + 78 - int((screen_resolution[1] - 110 - 4 * int((550.0 / 1280.0 * float(screen_resolution[0])) / (550.0 / 80.0)) - 3 * int((550.0 / 1280.0 * float(screen_resolution[0])) / (550.0 / 20.0)) + int(4.0 * (550.0 / 1280.0 * float(screen_resolution[0])) / (550.0 / 80.0) / 10.0) + int(15.0 / 80.0 * (550.0 / 1280.0 * float(screen_resolution[0])) / (550.0 / 80.0))) / 2.0));
    vec2 top_right_cell = vec2(int(10.0 / 1280.0 * float(screen_resolution[0])) + int(float(screen_resolution[0]) / 2.0), top_left_cell[1]);
    if (gl_FragCoord[0] >= 2 && gl_FragCoord[0] <= screen_resolution[0] - 3 && gl_FragCoord[1] >= 80 && gl_FragCoord[1] <= screen_resolution[1] - 35)
        ...
        {
            if (gl_FragCoord[0] >= top_left_cell[0] && gl_FragCoord[0] <= top_left_cell[0] + int(550.0 / 1280.0 * float(screen_resolution[0])) - 1)
            {
                gradient_coeff = (float(top_left_cell[0]) + 550.0 / 1280.0 * float(screen_resolution[0]) / 2.0 - float(gl_FragCoord[0])) / (550.0 / 1280.0 * float(screen_resolution[0]) / 2.0);
            ...
            else if (gl_FragCoord[0] >= top_right_cell[0] && gl_FragCoord[0] <= top_right_cell[0] + int(550.0 / 1280.0 * float(screen_resolution[0])) - 1)
            {
                gradient_coeff = (float(top_right_cell[0]) + 550.0 / 1280.0 * float(screen_resolution[0]) / 2.0 - float(gl_FragCoord[0])) / (550.0 / 1280.0 * float(screen_resolution[0]) / 2.0);
                schedule_coeff = (255.0 - abs(int(gradient_coeff * gradient_coeff * gradient_coeff * 255.0))) / 255.0;
                color_frag = vec4(vec3(schedule_coeff), real_schedule_opacity);
            }
            else
