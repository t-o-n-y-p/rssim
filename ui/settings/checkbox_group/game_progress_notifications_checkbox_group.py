...
@final
class GameProgressNotificationsCheckboxGroup(CheckboxGroup):
    def __init__(self, column, row, on_update_state_actions, parent_viewport):
        super().__init__(
            column, row, parent_viewport,
            logger=getLogger('root.app.settings.view.checkbox_group.game_progress_notifications_checkbox_group')
        )
        self.description_label = GameProgressNotificationsCheckboxGroupDescriptionLabel(parent_viewport=self.viewport)
