...
class InteractiveLabel(ABC):
    ...
    def create(self):
        self.placeholder_label = PygletLabel(
            self.get_formatted_placeholder_text(), font_name=self.font_name, bold=self.bold, font_size=self.font_size,
    ...
    def on_text(self, text):
        if len(self.text) == 0:
            ...
            self.text_label = PygletLabel(
                self.text, font_name=self.font_name, bold=self.bold, font_size=self.font_size,
                color=(*self.base_color, self.opacity), x=self.x, y=self.y, width=self.width,
                anchor_x=self.anchor_x, anchor_y=self.anchor_y, align=self.align, multiline=self.multiline,
                batch=self.batch, group=self.group
            )
    ...
    def on_key_press(self, symbol, modifiers):
        if symbol == BACKSPACE:
            if len(self.text) == 1:
                ...
                self.text = ''
                self.placeholder_label = PygletLabel(
                    self.get_formatted_placeholder_text(), font_name=self.font_name, bold=self.bold,
